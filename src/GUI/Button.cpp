...
namespace CGE
{
	namespace GUI
	{
		...
		void Button::draw()
		{
			if (visible_)
			{
				//Get the button position on the panel
				//Get the button model
				Loader::TexturedModel buttonModel = Loader::resManagement::getTexModel(type_);
				//Get the shader
