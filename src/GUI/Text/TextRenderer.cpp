...
#define setFontSize(size) FT_Set_Pixel_Sizes(font, 0, size)
                    ...
                    bool centered)
    {
        ...
        if (centered)
            position.x -= static_cast<float>(getStringLength(text, scale));
        else
            position.y -= characters[*text.begin()].size.y / (float) CGE::IO::DEFAULT_HEIGHT / displayScale.y *
        ...
        for (std::string::const_iterator c = text.begin(); c != text.end(); ++c)
        {
            ...
            const float xpos = position.x + (ch.bearing.x * scale) / CGE::IO::DEFAULT_WIDTH / displayScale.x * 2;
            /*
             ...
             */
            const float ypos =
                    position.y - scale * (17 + ch.size.y - ch.bearing.y) / CGE::IO::DEFAULT_HEIGHT / displayScale.y * 2;
            ...
            position.x += (ch.advance >> 6) * scale / displayScale.x / CGE::IO::DEFAULT_WIDTH *
